...
/**
 * Inserts a yes/no prompt inside a Truck Stops Nearby card.
 * The prompt is added as a child element of the card.
 */
function showCardWaypointPrompt(card, marker) {
  // Remove any existing prompt in the card.
  ...
  try {
    ...
    if (stationsInRange.length > 0) {
      stationsInRange.forEach((marker) => {
        ...
        `;
        // In route mode, clicking the card shows the infoWindow on the marker and then adds the waypoint prompt within the card.
...
async function highlightStationsAlongRoute(routePolyline) {
  ...
  if (stationsNearRoute.length > 0) {
    stationsNearRoute.forEach((marker) => {
      ...
      // In route mode, clicking the card will trigger the marker's infoWindow and add the waypoint prompt into the card.
      li.addEventListener("click", () => {
        if (document.getElementById("modeSelect").value === "route") {
          google.maps.event.trigger(marker, "click");
          showCardWaypointPrompt(li, marker);
