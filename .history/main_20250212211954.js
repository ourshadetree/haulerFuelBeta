...
function plotLocationsOnMap(map, locations) {
  ...
  locations.forEach((location) => {
    ...
    if (location.latP && location.lngP) {
      ...
      // Marker click: In route mode, prompt yes/no; otherwise show standard infoWindow.
      pilotMarker.addListener("click", () => {
        if (document.getElementById("modeSelect").value === "route") {
          showWaypointPrompt(pilotMarker);
        } else {
    ...
    if (location.latC && location.lngC) {
      ...
      caseyMarker.addListener("click", () => {
        if (document.getElementById("modeSelect").value === "route") {
          showWaypointPrompt(caseyMarker);
        } else {
          infoWindow.setContent(
            `<div>
 ...
 * This is used when the user clicks on a station card (or marker) in route mode.
 */
  ...
  try {
    ...
    if (stationsInRange.length > 0) {
      stationsInRange.forEach((marker) => {
        ...
        `;
        // In route mode, card click triggers the yes/no prompt;
        // otherwise, it triggers the marker click event.
