...
function plotLocationsOnMap(map, locations) {
  ...
  locations.forEach((location) => {
    ...
    if (location.latP && location.lngP) {
      ...
      // Always display the standard infoWindow with station details when clicked.
...
/**
 * Displays a yes/no prompt to mark the station as a waypoint.
 * This is intended to be triggered when a user clicks a card in the
 * Truck Stops Nearby list (after a route is created).
 */
function showWaypointPrompt(marker) {
  ...
  try {
    ...
    if (stationsInRange.length > 0) {
      stationsInRange.forEach((marker) => {
        ...
        `;
        // Updated click handler for list items:
        li.addEventListener("click", () => {
          if (document.getElementById("modeSelect").value === "route") {
            // In route mode, prompt for waypoint addition.
            showWaypointPrompt(marker);
          } else {
            // Otherwise, simply trigger the marker click event.
