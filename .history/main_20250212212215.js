...
function plotLocationsOnMap(map, locations) {
  ...
  locations.forEach((location) => {
    ...
    if (location.latP && location.lngP) {
      ...
      // Marker click: always show the standard infoWindow (even in route mode).
 ...
 * This is triggered when the user clicks a station card in route mode.
 */
  ...
  try {
    ...
    if (stationsInRange.length > 0) {
      stationsInRange.forEach((marker) => {
        ...
        `;
        // In route mode, card click triggers the yes/no waypoint prompt;
        // in non-route mode, it triggers the marker click event.
...
async function highlightStationsAlongRoute(routePolyline) {
  ...
  if (stationsNearRoute.length > 0) {
    stationsNearRoute.forEach((marker) => {
      ...
      li.addEventListener("click", () => {
        if (document.getElementById("modeSelect").value === "route") {
          // In route mode, station card click triggers the waypoint prompt.
          showWaypointPrompt(marker);
        } else {
          google.maps.event.trigger(marker, "click");
        }
