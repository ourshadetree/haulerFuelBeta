...
function filterStations(stations, userLocation) {
  ...
  if (selectedPrice && selectedPrice !== "all-prices") {
    ...
    if (selectedPrice.indexOf("-") !== -1) {
      ...
      if (parts.length === 2) {
        ...
        // If the second part is blank (e.g., "90-"), then use Infinity.
  ...
  // 3. Distance Filter
  // Our dropdown: "0" = Any Distance, "1" = 0-10 Miles, etc.
  const selectedDistance = document.getElementById("distance-filter").value;
  // If "0" (Any Distance) is selected, use Infinity; otherwise, calculate max distance.
  ...
  return stations.filter((marker) => {
    ...
    if (priceFilterActive) {
      if (stationPrice == null || isNaN(stationPrice)) {
        ...
        // Require the station price to be within the specified range.
